...
def save_video_stats(bv_id: str, video_info: list, stat: dict, time_str: str | None = None) -> dict:
    """
    保存一次视频统计快照，更新历史记录，数据足够时重绘趋势图。
    time_str: 同一次任务统一的时间目录名；不传则内部生成。
    """
    ...
    snapshot_path = snap_dir / "stats_analysis.json"
    ...
    log_event(
        ...
        bv_id=bv_id, snapshot_path=str(snapshot_path),
        history_path=str(history_path), point_count=len(history["records"]),
        stat=full_stat,
    )
